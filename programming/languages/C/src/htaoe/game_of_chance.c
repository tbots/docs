...
#define DATAFILE		"/var/cache.data"			// File to store user data
...
/* Print msg following perror()-generated error message and exit with 
...
int main() {
	...
	if (get_player_data() == -1)			// Try to read player data from the file.
...
int get_player_data() {
	...
	if ( fd == -1 )			// Cann't open the file, maybe it doesn't exist
		return -1;
...
void register_new_player() {
	...
	if(fd == -1)
		fatal("in register new_player() while opening file");
	if(write(fd, &player, sizeof(struct user)) < sizeof(struct user))
		fatal("in register new_player() while writing structure");
	close(fd);
...
// This function will display the current highscore and 
// the name of the person who set that highscore.
void show_highscore() {
...
// returns -1, if the wager is too big or too little, and it returns the  wager amount 
// otherwise. 
...
// played again. It also writes the new credits totals to file after 
// each game is played. 
void play_the_game() {
	int play_again = 1;
	char selection;
	...
	while(play_again) {
		...
		if(player.current_game() != -1) {				// If the game plays withour error and
...
int find_the_ace() {
	...
	cards[i] = 'Q';
	print_cards("Revealing the queen",cards,pick);
	invalid_choice = 1;
	while(invalid_choice) {					// Loop until valid choice is made.
		...
		if(pick == ace)	{						// Handle win.
			...
			printf("You have lost %d credits from your second wager!\n",wager_two);
			player.credits -= wager_one;
			if(wager_two != -1) {
				printf("and an additional %d credits from your second wager!\n",wager_two);
				player.credits -= wager_two;
