#!/bin/bash
#
# run.sh			runs deployment script on the remote machine
#
# improvemenet:	do not rewrite keys
# issue:		check .dep file before passing it to source
...
then
	...
	echo "Script requires .dep file to be specified where it lists all required "
	echo "variables for the further deployment."
	echo 
	echo "Options:"
...
# need to test if all the variables are set
source $CONF_DEP
source $PRJ/$VARS		
...
then
	#
	# copy directory with deployment scripts on the remote host
	#
	echo "Copying \``basename $PRJ_PTH`/' directory to \`$DEP_HOST'..."
	scp -r $PRJ_PTH root@$DEP_HOST:/root 1> /dev/null || { echo "failed to copy script"; exit 1; }

	# issue script on remote side
